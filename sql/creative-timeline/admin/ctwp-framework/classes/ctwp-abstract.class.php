 ...
 * 
 */
if (!class_exists('CTWP_Abstract')) {
  abstract class CTWP_Abstract {
    ...
    public function __construct() {
      if (!empty($this->args['output_css']) || !empty($this->args['enqueue_webfont'])) {
        ...
        CTWP::$css = apply_filters("ctwp_{$this->unique}_output_css", CTWP::$css, $this);
      }
    }
    ...
    public function recursive_output_css($fields = array(), $combine_field = array()) {
      if (!empty($fields)) {
        foreach ($fields as $field) {
          ...
          $field_class  = 'CTWP_Field_' . $field_type;
